...
def process_pdf(input_file: Path, output_file: Path, gap_size: int = 20, layout: str = "side-by-side"):
    """
    ...
    try:
        ...
        for i in range(0, total_pages, 2):
            ...
            if i + 1 < total_pages:
                ...
                a4_page = arrange_pages_on_a4_rotated(page1, page2, gap_size, layout)
                writer.add_page(a4_page)
            else:
                # Only one page left, create A4 with just that page
                a4_page = arrange_pages_on_a4_rotated(page1, None, gap_size, layout)
                writer.add_page(a4_page)
...
def main():
    ...
    parser.add_argument("--layout", choices=["side-by-side", "top-bottom"], default="side-by-side", help="Layout orientation: side-by-side or top-bottom (default: side-by-side)")
    ...
    # Process the PDF
    result = process_pdf(input_path, output_path, args.gap, args.layout)
